...
#include "../../drivers/staging/imx-drm/imx-hdmi.h"
...
static irqreturn_t dw_hdmi_cec_irq(int irq, void *data)
{
	...
	printk(KERN_ERR "irq : %x", stat);
	if (stat == 0)
		return IRQ_NONE;

	writeb(stat, cec->base + HDMI_IH_CEC_STAT0);

	if (stat & (CEC_STAT_DONE | CEC_STAT_NACK | CEC_STAT_ERROR_INIT))
...
/* FIXME begin stub
 * substitution enable/disable functions
 * as no correct wiring with HDMI driver for now
 */
static void imx_hdmi_cec_enable(void *data)
{
	...
	val = readb(cec->base + HDMI_MC_CLKDIS);
	val &= ~HDMI_MC_CLKDIS_CECCLK_DISABLE;
	writeb(val, cec->base + HDMI_MC_CLKDIS);
}
...
static int dw_hdmi_cec_probe(struct platform_device *pdev)
{
	...
	cec->dev = &pdev->dev;
	cec->base =  devm_ioremap(&pdev->dev, 0x00120000, 0x9000); //
	cec->irq = platform_get_irq(pdev, 0); // data->irq;
	cec->ops = &imx_hdmi_cec_ops; // data->ops
	cec->ops_data = cec; // data->ops_data;
...
static const struct of_device_id dw_hdmi_cec_match[] = {
...
static struct platform_driver dw_hdmi_cec_driver = {
	...
	.driver = {
		.name = "dw-hdmi-cec",
		.owner = THIS_MODULE,
		.of_match_table	= dw_hdmi_cec_match,
	},
};

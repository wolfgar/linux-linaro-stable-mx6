...
// FIXME : for now no integration with HDMI driver
	...
	size_t count)
{
	...
	cec_dev->write_state = CEC_DEV_WRITE_BUSY;
	...
	__acquires(cec_dev->mutex)
{
	...
	do {
		if (file->f_flags & O_NONBLOCK) {
			if (cec_dev->write_state != CEC_DEV_WRITE_AVAIL)
				return -EAGAIN;
		} else {
			ret = wait_event_interruptible(cec_dev->waitq,
						       (cec_dev->write_state == CEC_DEV_WRITE_AVAIL));
			if (ret)
				break;
		}
	...
	size_t count, loff_t *ppos)
{
	...
	ret = wait_event_interruptible_timeout(cec_dev->waitq,
				(cec_dev->write_state != CEC_DEV_WRITE_BUSY),
				msecs_to_jiffies(1000));

	// Locking for write_state accesses seems unecessary here (TBC)
	mutex_lock(&cec_dev->mutex);
	spin_lock_irqsave(&cec_dev->lock, flags);

	if (ret < 0)
		count_ret = -ERESTARTSYS;
	else
		count_ret = (cec_dev->write_state == CEC_DEV_WRITE_OK) ?
								count : -EIO;
...
static long cec_dev_ioctl(struct file *file, u_int cmd, unsigned long arg)
{
	...
	case HDMICEC_IOC_GETPHYADDRESS:
		/* FIXME for now no proper integration with HDMI driver
...
static int cec_dev_release(struct inode *inode, struct file *file)
{
	...
	if (cec_dev->users == 0) {
		 ...
		 */
		wait_event_timeout(cec_dev->waitq, cec_dev->write_state == CEC_DEV_WRITE_AVAIL,
